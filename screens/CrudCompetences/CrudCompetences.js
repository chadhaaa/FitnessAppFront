...
const CrudCompetences = () => {
  ...
  const getComps = async () => {
    ...
    const response = await axios
      .get("http://192.168.108.64:8000/api/competences", { headers })
      .then((res) => {
        console.log(res);
        setComp(res.data);
  ...
  const handleDelete = (item) => {
    ...
    const response = axios
      .delete("http://192.168.108.64:8000/api/competence/" + item._id, {
        headers,
      })
      .then((res) => {
        var respo = res.data;
  ...
  const handleSave = () => {
    if (hideId == null) {
      ...
      axios
        .post("http://192.168.108.64:8000/api/competence", formdata)
        .then((res) => {
      ...
      axios
        .put("http://192.168.108.64:8000/api/competence/" + hideId, formdata)
        .then((res) => {
          const response = res.data;
          getComps();
          setName("");
