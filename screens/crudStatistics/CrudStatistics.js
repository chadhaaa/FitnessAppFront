...
const CrudStatistics = () => {
  ...
  const getStats = async () => {
    ...
    const response = await axios
      .get("http://10.1.0.130:8000/api/statistics", { headers })
      .then((res) => {
        console.log(res);
        setStat(res.data);
  ...
  const handleDelete = (item) => {
    ...
    const response = axios
      .delete("http://10.1.0.130:8000/api/statistic/" + item._id, {
        headers,
      })
      .then((res) => {
        var respo = res.data;
  ...
  const handleSave = () => {
    if (hideId == null) {
      ...
      axios
        .post("http://10.1.0.130:8000/api/statistic", formdata)
        .then((res) => {
      ...
      axios
        .put("http://10.1.0.130:8000/api/statistic/" + hideId, formdata)
        .then((res) => {
          const response = res.data;
          getStats();
          setTitle("");
